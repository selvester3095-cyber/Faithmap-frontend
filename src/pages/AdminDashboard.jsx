...
export default function AdminDashboard() {
  ...
  const handleVerify = async (church) => {
    if (!confirm(`Verify "${church.name}"? It will go live on faithmap.in`)) return
    try {
            ...
            <div key={church.id} className="card p-5">
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500">
                    {church.area}, {church.city} · {church.denomination || 'No denomination'}
                  </p>
                  <div className="grid grid-cols-2 gap-x-6 gap-y-2 mt-3 text-sm">
                    {church.contact_name && (
                      <div>
                        <span className="text-xs text-slate-400 uppercase">Contact</span>
                        <p className="text-slate-700">{church.contact_name}</p>
                      </div>
                    )}
                    {church.phone && (
                      <div>
                        <span className="text-xs text-slate-400 uppercase">Phone</span>
                        <p className="text-slate-700">{church.phone}</p>
                      </div>
                    )}
                    {church.email && (
                      <div>
                        <span className="text-xs text-slate-400 uppercase">Email</span>
                        <p className="text-slate-700">{church.email}</p>
                      </div>
                    )}
                    {church.address && (
                      <div>
                        <span className="text-xs text-slate-400 uppercase">Address</span>
                        <p className="text-slate-700">{church.address}</p>
                      </div>
                    )}
                  </div>
                  {church.description && (
                    <p className="text-sm text-slate-500 mt-2 italic">{church.description}</p>
                  )}
